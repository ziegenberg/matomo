...
namespace Matomo\Tracker\Handler;
...
class Factory
{
    public static function make()
    {
         ...
         * {@link Matomo\Tracker\Handler} and customize any tracking behavior.
         *
         * @param \Matomo\Tracker\Handler &$handler Initialized to null, but can be set to
         ...
         */
        Matomo::postEvent('Tracker.newHandler', array(&$handler));

        if (is_null($handler)) {
            $handler = new Handler();
        } elseif (!($handler instanceof Handler)) {
            throw new Exception("The Handler object set in the plugin must be an instance of Matomo\\Tracker\\Handler");
        }
