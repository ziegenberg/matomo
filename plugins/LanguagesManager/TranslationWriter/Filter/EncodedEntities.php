        ...
        foreach ($translations as $pluginName => $pluginTranslations) {
            foreach ($pluginTranslations as $key => $translation) {
                if (
                    ...
                    $this->baseTranslations[$pluginName][$key] != StaticContainer::get('Matomo\Translation\Translator')->clean($this->baseTranslations[$pluginName][$key])
                ) {
                    continue; // skip if base translation already contains encoded entities
                }

                // remove encoded entities
                $decoded = StaticContainer::get('Matomo\Translation\Translator')->clean($translation);
